...
from network import NMT
from utils import Hypothesis, batch_iter, read_corpus
...
def opt_search(model: NMT, test_data_src: List[List[str]], chunk_size: int, opt_lr: float, opt_step: int, ent_reg: float, max_decoding_time_step: int) -> List[List[str]]:
    was_training = model.training
    model.eval()
...
def opt_decode(args: Dict[str, str]):
    ...
    hypotheses = opt_search(model, test_data_src,
